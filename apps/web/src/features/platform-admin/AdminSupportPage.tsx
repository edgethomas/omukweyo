  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-2 sm:flex-wrap">
        <div>
          <h2 className="text-[16px] sm:text-[18px] font-semibold text-ink">Support</h2>
          <p className="text-[11px] sm:text-[12px] text-ink-3 mt-0.5">Read-only support view of the SMS log</p>
        </div>
        <div className="relative w-full sm:w-auto">
          <Search size={13} className="text-ink-3 absolute left-2.5 top-1/2 -translate-y-1/2" />
          <input className="input h-9 pl-8 text-[12px] w-full sm:w-64" placeholder="Search by phone or template" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      </div>
      ...
      <section className="card p-0 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-6 sm:p-8 text-center text-[12px] text-ink-3">No messages match.</div>
        ) : (
          <div className="divide-y divide-line max-h-[36rem] overflow-auto">
            {filtered.slice(0, 100).map((n) => (
              <div key={n.id} className="px-4 sm:px-5 py-2.5 text-[12px]">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="font-mono text-[10px] text-ink-3 sm:ml-auto">{relativeTime(n.at)}</span>
                </div>
                <p className="text-ink-2 mt-1.5 leading-relaxed break-words">{n.message}</p>
      ...
      <section className="card p-4 sm:p-5 text-[12px] text-ink-2 inline-flex items-center gap-2">
        <ShieldAlert size={14} className="text-accent" />
        Account intervention tools require audited admin approval before use.
      </section>
    </div>
  );
}
