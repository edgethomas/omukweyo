    ...
    <div className="space-y-5">
      <div className="flex items-center gap-2">
        <div>
          ...
          <h2 className="text-[16px] font-semibold text-ink">All roles, queues, reservations, and onboarding activity</h2>
        </div>
        <Link to="/login" className="btn btn-outline btn-sm ml-auto">Role login</Link>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <section className="lg:col-span-2 card p-0 overflow-hidden">
          ...
          <div className="divide-y divide-line">
            {data.roleCoverage.map((role: any) => (
              <Link key={role.role} to={role.entry} className="grid sm:grid-cols-[1fr_120px_140px] gap-3 px-5 py-3 hover:bg-surface-2">
                <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-[12px] text-ink-3 sm:text-right">{role.entry}</span>
              </Link>
            ))}
          </div>
        </section>
        ...
        <aside className="space-y-4">
          <div className="card p-5">
            ...
            <div className="t-mono text-3xl text-ink font-semibold mt-2">{totals.notifications}</div>
            <p className="text-[12px] text-ink-2 mt-1">SMS and support events logged by the demo API.</p>
          </div>
          <div className="card p-5">
            <div className="t-eyebrow mb-2">Business onboarding</div>
            <div className="t-mono text-3xl text-ink font-semibold">{totals.businessOnboardings}</div>
            <Link to="/onboarding" className="btn btn-outline btn-sm mt-4">Open business setup</Link>
          </div>
          <div className="card p-5">
            <div className="t-eyebrow mb-2">Runner applications</div>
            <div className="t-mono text-3xl text-ink font-semibold">{totals.runnerApplications}</div>
            <Link to="/runner/signup" className="btn btn-outline btn-sm mt-4">Open runner signup</Link>
          </div>
        </aside>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <section className="card p-0 overflow-hidden">
          ...
          <div className="divide-y divide-line">
            {data.liveBranches.map((branch: any) => (
              <div key={branch.id} className="px-5 py-3 grid grid-cols-[1fr_90px_90px] gap-3 items-center">
                <div>
                  <div className="text-[13px] font-semibold text-ink">{branch.name}</div>
                  <div className="text-[11px] text-ink-3">{branch.city}</div>
                </div>
                <div className="t-mono text-[12px] text-ink-2">{branch.avgWaitMin}m avg</div>
        ...
        <section className="card p-0 overflow-hidden">
          ...
          {data.recentBusinessOnboardings.length === 0 ? (
            <div className="p-6 text-[12px] text-ink-3">No business onboarding records yet. Use Sign up to create one.</div>
          ) : (
            <div className="divide-y divide-line">
              {data.recentBusinessOnboardings.map((item: any) => (
                <Link key={item.id} to={item.launchLinks.dashboard} className="px-5 py-3 block hover:bg-surface-2">
        ...
        <section className="card p-0 overflow-hidden">
          ...
          {data.recentReservations.length === 0 ? (
            <div className="p-6 text-[12px] text-ink-3">No future reservations yet. Use the customer signup flow to create one.</div>
          ) : (
            <div className="divide-y divide-line">
              {data.recentReservations.map((reservation: any) => (
                <Link key={reservation.id} to={`/reservation/${reservation.id}`} className="px-5 py-3 grid grid-cols-[1fr_90px] gap-3 hover:bg-surface-2">
                  <div>
                  ...
                  <div className="text-right t-mono text-[12px] text-ink-3">{formatTime(reservation.targetArrivalAt)}</div>
                </Link>
              ))}
            </div>
          )}
...
function Kpi({ icon, label, value }: { icon: ReactNode; label: string; value: string | number }) {
  return (
    <div className="bg-surface p-4">
      <div className="flex items-center gap-1.5 t-eyebrow text-[10px]">{icon}{label}</div>
